...
const colAccent = {
  todo:       { bar: "bg-violet-500",  count: "bg-violet-100 text-violet-600",  header: "border-t-violet-500" },
  inProgress: { bar: "bg-blue-500",    count: "bg-blue-100 text-blue-600",      header: "border-t-blue-500" },
  review:     { bar: "bg-amber-400",   count: "bg-amber-100 text-amber-600",    header: "border-t-amber-400" },
  done:       { bar: "bg-emerald-500", count: "bg-emerald-100 text-emerald-600",header: "border-t-emerald-500" },
};

const fallback = { bar: "bg-indigo-500", count: "bg-indigo-100 text-indigo-600", header: "border-t-indigo-500" };

function Column({ id, column, onDeleteTask, onEditTask, onDeleteColumn, dragItem, dragColumn, onDropTask }) {
  ...
  return (
    <div
      onDrop={() => { onDropTask(id); setIsDragOver(false); }}
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-700 tracking-tight">{column.name}</h3>
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${accent.count}`}>
            {column.items.length}
          </span>
        </div>
        ...
        <div className="relative">
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-20 bg-white border border-slate-200 rounded-xl shadow-lg py-1 w-36">
              <button
                onClick={() => { onDeleteColumn(id); setMenuOpen(false); }}
      ...
      <div className="flex flex-col gap-2.5 p-3 flex-1 min-h-24 overflow-y-auto max-h-[calc(100vh-220px)]">
        {column.items.length === 0 ? (
          <div className={[
            ...
            isDragOver ? "border-indigo-300 bg-indigo-50" : "border-slate-200",
          ].join(" ")}>
...
const Columns = ({ columns, onDeleteTask, onEditTask, onDeleteColumn, dragItem, dragColumn, onDropTask }) => {
  return (
